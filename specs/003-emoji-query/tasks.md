...
- [ ] T012 [US1] `nekochan_suggest/query.py` に `_embed_text(text: str, embed_model: str) -> list[float]` を実装する。`sentence_transformers.SentenceTransformer(embed_model).encode(text)` を使用し、`.tolist()` で `list[float]` に変換する。`encode()` 結果が空または次元数が 0 でないことを検証する。`OSError`・`RuntimeError`・`ValueError` は呼び出し元に伝播する
