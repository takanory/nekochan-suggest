...
ユーザーが入力したテキストを `sentence-transformers` パッケージで埋め込みベクトルに変換し、
...
埋め込み生成には `sentence-transformers` PyPI パッケージ、設定読み込みには標準ライブラリ `tomllib` を使用する。
...
**Testing**: `pytest`, `pytest-cov`, `unittest.mock`（CI 上でモデルダウンロード不要）  
**Target Platform**: macOS / Linux CLI  
**Project Type**: CLI ツール + ライブラリ API  
**Performance Goals**: 埋め込み生成を除く処理（入力解析・類似度計算・出力整形）が 1 秒以内（SC-001）  
**Constraints**: 追加 PyPI 依存は `sentence-transformers` のみ。コサイン類似度は `math` 標準ライブラリのみで純 Python 実装  
...
- [x] **I. Python ファースト・シンプリシティ** — Python 3.11+（憲法推奨 3.13+ に近い）。
  新規サードパーティ依存は `sentence-transformers` のみ（埋め込み生成という具体的要件で正当化）。
...
- [x] **I. Python ファースト・シンプリシティ** — `query.py`（クエリロジック）・`cli.py`（入出力）の
  ...
  新規依存 `sentence-transformers` は埋め込み生成という具体的要件により正当化。
