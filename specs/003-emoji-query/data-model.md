...
| `count` | `int` | `3` | 1＞10 の整数。0 以下または 11 以上は `Error: --count out of range (1-10).` を stderr に出力して終了コード 1 |

**`json_mode`**: `suggest()` には渡さない。CLI 層が処理する。

**入力優先順位**: コマンドライン引数（TEXT）と stdin の両方が指定された場合はコマンドライン引数を優先する（CLI 層の責務）。
