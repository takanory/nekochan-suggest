...
**テストについて**: 仕様書でテストは明示要求されていないため、US3（テスト実行環境）の実装タスクとして統合。
...
- [ ] T003 [US1] cli.py を実装する（argparse でヘルプ・テキスト位置引数・`build-annotations` サブコマンド・`--count`/`--json`/`--timeout` オプション・スタブ応答出力・型ヒント・日本語 docstring）`nekochan_suggest/cli.py`
- [ ] T004 [P] [US1] ui.py を実装する（`main()` でプレースホルダーメッセージを出力・`--help` 対応・型ヒント・日本語 docstring）`nekochan_suggest/ui.py`
...
## Phase 4: US3 — 開発環境でテストが実行できる（優先度: P3）

**Goal**: `uv run pytest` または `make test` でテストスイートが実行されパスする

**独立テスト**: `make test` を実行し、少なくとも 1 件のテストがパスすれば完結

- [ ] T007 [US3] tests/test_cli.py を作成する（パッケージインポート・`--help` 出力検証・`build-annotations` サブコマンド認識・終了コード 0 の基本テスト）`tests/test_cli.py`

**チェックポイント**: `uv run pytest` がエラーなく実行され、全テストがパスすること

---

## Phase 5: US4 — リント・型チェック・フォーマットを 1 コマンドで実行できる（優先度: P3）

**Goal**: `make check` 1 コマンドで lint → typecheck → test が順に実行され、すべてエラーゼロで完了する

**独立テスト**: `make check` を実行し、初期スタブコードに対してエラーゼロで完了すれば完結

- [ ] T008 [US4] pyproject.toml に ruff・pyrefly 設定を追加する（`[tool.ruff]` target-version/line-length・`[tool.ruff.lint]` select=E/F/I/N/UP・`[tool.pyrefly]` project-includes）`pyproject.toml`
- [ ] T009 [P] [US4] Makefile を作成する（`lint`/`format`/`typecheck`/`test`/`check` の各ターゲットを `uv run` 経由で定義）`Makefile`
...
- [ ] T011 make check を実行して全 SC を検証する（SC-001〜SC-005 を順に確認）
...
- **Phase 2（US1）**: Phase 1 完了後に開始（T001・T002 完了が前提）
- **Phase 3（US2）**: Phase 2 完了後に開始（T003 が `cli.py` に build-annotations を定義済みであること）
- **Phase 4（US3）**: Phase 3 完了後に開始（テスト対象のスタブが存在すること）
- **Phase 5（US4）**: Phase 4 完了後に開始（テストが通る状態でリント設定を追加）
- **Phase 6（ポリッシュ）**: Phase 5 完了後 — T010 は並行開始可能

### ユーザーストーリー依存

- **US1（P1）**: Phase 1 完了後すぐ開始可能
- **US2（P2）**: US1 完了後（T003 で `cli.py` の骨格が存在すること）— US1 と独立してテスト可能
- **US3（P3）**: US2 完了後（テスト対象の全モジュールが存在すること）
- **US4（P3）**: US3 完了後（`make test` が通る状態でリント設定を追加）
...
- **Phase 5（US4）**: T008 と T009 は並行実行可能（`pyproject.toml` と `Makefile`）
...
# Task: T004 — ui.py を実装する  ← 同時実行可能（異なるファイル）
```
...
3. Phase 4（US3）追加 → テストが実行可能 ✅
4. Phase 5（US4）追加 → CI 対応のコード品質チェックが整備 ✅
5. Phase 6（ポリッシュ）→ README と最終確認 ✅
